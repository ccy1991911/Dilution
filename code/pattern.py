...
def find_api(call_id,next_id,words):
    ...
    for i in range(call_id+1,next_id):
        api+=words[i]
        if word_type(words[i],[api_list])>0: break
    return api
...
verify_verbs=filetolist('../data/configure/verb/verify')
check_contents=file_to_matrix('../data/configure/check_content',',')
consistent=filetolist('../data/configure/consistent')